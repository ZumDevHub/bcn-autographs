...
export default function ContextImagesCarrousel({ imageUrl, alt, imagesArr }: ContextImageCarrouselProps) {
  ...
  return (
      ...
      <div className="relative flex justify-around w-full h-full flex items-center justify-center p-4 z-30 cursor-pointer">
        ...
        {imagesArr.length > 1 &&
        <div className="absolute bottom-5 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex justify-evenly items-center w-2/5 bg-black/80 rounded-lg py-1">
          {imagesArr.map((img, ind:number) => {
            circleMenu.push(ind)
            return(
              <div key={img.id} onClick={()=> setIndex(ind)}>
                <div className={ind === index ? "w-4 h-4 bg-gray-200 rounded-full border-2 border-white" : "w-2 h-2 bg-gray-200 rounded-full hover:bg-gray-400"}></div>
              </div>
            )
          })}
        </div>}
